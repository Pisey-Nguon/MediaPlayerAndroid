...
class PrepareDownloadService:Service() {
            ...
            resource.subtitles.forEachIndexed {index, subtitleModel ->
                TaskRunner().executeAsync(DownloadFile(this, subtitleModel.subtitleUrl), object : TaskRunner.Callback<String?> {
                    override fun onComplete(result: String?) {
                        if (result != null){
                            clone?.subtitles?.get(index)?.subtitleUrl = result
                        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val playerResource = intent?.getParcelableExtra<PlayerResource>(Constants.EXO_PLAYER_RESOURCE)
        val trackIndexMovie = intent?.getIntExtra(Constants.EXO_TRACK_INDEX,0)
